...
def update(grid_lib, params):
    # Update grid parameters based on params
    nfin = np.ceil(params['w']/100)
    nfos = 100
    height = 2*(600 + nfos*nfin)
    yofst  = nfos*(nfin-4)
    r12 = grid_lib['routing_12_cmos']
    r23 = grid_lib['routing_23_cmos']
    r12y = r12.elements[1]
    r23y = r23.elements[1]
    # Shift y coordinates
    for i in range(1, 8):
        ...
        r23y[i] += yofst 
    # Adjust grid height
    r12._xy[1].range[1]=height
    r23._xy[1].range[1]=height
    return grid_lib
